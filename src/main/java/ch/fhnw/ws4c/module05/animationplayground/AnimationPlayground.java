...
package ch.fhnw.ws4c.module05.animationplayground;
...
public class AnimationPlayground extends BorderPane {
	...
	private void initializeControls() {
		...
		path.getStrokeDashArray().setAll(20d, 5d);

		pathFinder = new Rectangle(30, 30, Color.CORNFLOWERBLUE);
	...
	private void addEventHandlers() {
		rectangle.setOnMouseClicked(event -> {
			...
			if (translateTransition.getCurrentTime().equals(Duration.ZERO)) {
		...
		circle.setOnMouseClicked(event -> {
			...
			if (scaleTransition.getCurrentTime().equals(Duration.ZERO)) {
		...
		text.setOnMouseClicked(event -> {
			...
			if (rotateTransition.getCurrentTime().equals(Duration.ZERO)) {
		...
		line.setOnMouseClicked(event -> {
			...
			if (fadeTransition.getCurrentTime().equals(Duration.ZERO)) {
				fadeTransition.play();
			}
		});
		...
		pathFinder.setOnMouseClicked(event -> {
			...
			if (pathTransition.getCurrentTime().equals(Duration.ZERO)) {
				pathTransition.play();
			}
		});
		...
		ellipse.setOnMouseClicked(event -> {
			...
			if (fillTransition.getCurrentTime().equals(Duration.ZERO)) {
