package ch.fhnw.ws4c.module09.controllibrary;
...
class LinearSkin extends SkinBase<NumberRangeControl> {
	...
	private static final double MINIMUM_WIDTH  = PREFERRED_HEIGHT * 2.5;
	...
	private void initializeParts() {
		value = new Text(0, PREFERRED_HEIGHT * 0.5, String.format(FORMAT, getSkinnable().getValue()));
		value.getStyleClass().add("value");
		value.setTextOrigin(VPos.CENTER);
		value.setTextAlignment(TextAlignment.CENTER);
		value.setMouseTransparent(true);

		thumb = new Circle(PREFERRED_HEIGHT * 0.5, PREFERRED_HEIGHT * 0.5, PREFERRED_HEIGHT * 0.5);
	...
	private void resize() {
		...
		double scalingFactorY = actualHeight / PREFERRED_HEIGHT;
